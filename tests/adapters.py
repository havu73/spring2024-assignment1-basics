...
def run_multihead_self_attention(
    ...
    # gpt = MultiHeadSelfAttention(d_model, num_heads) # , d_k, d_v)
    # gpt.load_state_dict(weights)  # load in WQ, WK, WV, WO
    # gpt_out = gpt(in_features)
    my_model = MultiHead(d_model, num_heads, d_k, d_v)
    my_model.load_state_dict(weights)
    my_out = my_model(in_features)
    return my_out
...
def run_transformer_lm(
    ...
    lm = TransformerLM(vocab_size, context_length, d_model=d_model, embed_pdrop= 0,
                       num_heads=num_heads, d_ff=d_ff, attn_pdrop=attn_pdrop, residual_pdrop=residual_pdrop, num_layers=num_layers)
    lm.load_state_dict(weights)
    output = lm(in_indices)
    return output
...
def run_get_batch(
    ...
    context_windows, labels = get_batch(dataset, batch_size, context_length, device)  # batch_size, context_length
